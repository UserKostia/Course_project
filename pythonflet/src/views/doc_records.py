...
from components.userAvatar.index import get_avatar
...
def doc_records_view(_):
    ...
    def display_docs_records():
        ...
        c.execute("""
            SELECT 
            name, surname, middle_name, complaint, doc_full_name, registration_date, registration_time
            FROM patients
            """)
        patient_rec = c.fetchall()

        for index, rec in enumerate(patient_rec):
            name_val, surname_val, middle_name_val, complaint_val,\
